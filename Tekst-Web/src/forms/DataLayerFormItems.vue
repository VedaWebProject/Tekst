...
import DataLayerConfigFormItems from '@/forms/layers/config/DataLayerConfigFormItems.vue';
...
<template>
  <div class="content-block">
    <h3>{{ $t('dataLayers.headingGeneral') }}</h3>
    <!-- TITLE -->
  ...
  <div v-if="model.config" class="content-block">
    <DataLayerConfigFormItems
      :model="model.config"
      :layer-type="model.layerType"
      @update:model="(v: AnyLayerConfig) => handleUpdate('config', v)"
    />
  </div>

  <!-- ACCESS SHARES -->
  <div v-if="sharingAuthorized && model.sharedRead && model.sharedWrite" class="content-block">
    <h3>{{ $t('models.layer.share') }}</h3>
    <div
      v-if="public"
      style="
      ...
      "
    >
      {{ $t('dataLayers.edit.onlyForUnpublished') }}
    </div>
