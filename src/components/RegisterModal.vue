...
import { AuthApi, ResponseError } from 'textrig-ts-client';
...
async function registerUser() {
  try {
    await authApi.registerRegister({ userCreate: UserCreateFromJSON(formModel.value) });
    ...
    if (status == 400) {
      if (r.detail === 'REGISTER_USER_ALREADY_EXISTS') {
        messages.error('This email address is already registered.');
      } else if (r.detail.code && r.detail.code === 'REGISTER_INVALID_PASSWORD') {
